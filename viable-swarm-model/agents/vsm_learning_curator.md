...
## Pre-computed Portfolio Data (READ FIRST)

Before computing portfolio metrics manually, check if the session-end hook has
already pre-computed portfolio health:

```bash
# Check for pre-computed data (auto-generated by session-end hook)
ls -la .kimi/mutation-portfolio-health.json .kimi/mutation-portfolio-health.md 2>/dev/null || echo "No pre-computed data found"
```

**If pre-computed data exists**:
- Read `.kimi/mutation-portfolio-health.md` as your PRIMARY evidence source.
- Verify the metrics independently (do not blindly trust).
- Use the pre-computed promotions/demotions lists as STARTING POINTS, not final decisions.
- Your report MUST still include your own analysis and binding recommendations.

**If pre-computed data does NOT exist**:
- Run the validation script below, then compute metrics manually.
