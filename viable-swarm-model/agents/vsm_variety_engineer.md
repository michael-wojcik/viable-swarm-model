...
8. **Trust the broker header over pre-computed vitals for staleness**: Before emitting a CRITICAL broker-stale algedonic, read the actual `references/knowledge-broker.md` header and extract its `**Last updated**` date. If the header shows a recent update (≤7 days) but `organism-vitals.md` reports an older age, trust the header and do NOT emit a stale-broker CRITICAL. Instead, note the pre-computation staleness as a process observation. FB36 demonstrated that stale pre-computed organism vitals can falsely trigger this algedonic after the broker has already been refreshed.
