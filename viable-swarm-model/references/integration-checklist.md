...
### Check 1: Build Verification (BLOCKER)
...
### Check 3: Export/Import & Dead Code Scan (ISSUE)
...
### Check 4: File Structure & Documentation (ISSUE)
- [ ] No empty files committed
- [ ] README has setup instructions

### Check 5: Parallel Agent Coordination (BLOCKER)
...
### Check 6: Circular Import Prevention (BLOCKER)
...
- [ ] After any fix agent adds cross-module imports, run `python -c "import app.main"` (or equivalent) to verify no circular dependencies were introduced
...
### Check 7: Subprocess Import Verification (BLOCKER)
...
### Check 8: Dependency Verification (BLOCKER)
...
- [ ] For every third-party import (not `typing`, `datetime`, `os`, `json`, etc.), verify the
  package name appears in `requirements.txt` / `package.json` / equivalent manifest
- [ ] If a package is imported but NOT in the manifest, this is a BLOCKER
- [ ] Common trap packages to watch for: `strawberry_sqlalchemy_mapper`, `sqlalchemy_strawberry`,
  `fastapi_graphql_auto`
- [ ] After any Phase 0 environment fix (e.g., upgrading a package), verify the manifest
...
causing ~15 min agent timeout before S5 intervention (H150). FB23 Phase 0 upgraded
`strawberry-graphql` but `requirements.txt` still specified old version, causing clean-install
failures.

### Check 9: Zero Deprecation Warnings From Application Code (HIGH)
...
### Check 10: Phase 4 Exit Gate — Zero Test Failures Before Security/Integration (BLOCKER)
...
### Check 12: Re-Audit Report Artifact Exists After Fix Wave (BLOCKER)
...
### Check 13: Phase 8b Meta-Report Completeness (BLOCKER)
...
### Check 14: Runtime API Verification (BLOCKER)
...
### Check 15: Cross-Layer Runtime Consistency (BLOCKER)
...
- [ ] **Rust**: Workspace `Cargo.toml` includes ALL member crates; no duplicate imports between
  ...
  not local modules
- [ ] **Go**: All JSON tags are camelCase (`json:"camelCase"`); no snake_case leaking to frontend
  via struct tags
